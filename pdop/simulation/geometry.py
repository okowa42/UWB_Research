...
def dop_components(anchor_positions, tag_position, distances=None):
    """Decompose the position dilution of precision into PDOP/HDOP/VDOP.

    PDOP is the overall (3D) position DOP, HDOP covers the horizontal (x, y)
    plane, and VDOP covers the vertical (z) axis. VDOP is None when the
    geometry has fewer than 3 dimensions.
    """
    try:
        covariance = covariance_matrix(geometry_matrix(anchor_positions, tag_position, distances))
        pdop = np.sqrt(np.trace(covariance))
        if covariance.shape[0] >= 2:
            hdop = np.sqrt(covariance[0, 0] + covariance[1, 1])
        else:
            hdop = np.sqrt(covariance[0, 0])
        vdop = np.sqrt(covariance[2, 2]) if covariance.shape[0] >= 3 else None
